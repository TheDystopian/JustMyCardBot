...
class botUtils:
        ...
        return " ".join(
            filter(
                ...
                [
                    ...
                    "(За повторки)"
                    if cardData.get("repeat")
                    else f"({botUtils.formatStats({'scraps': cardData.get('scrapCost')})[0]})"
                    if cardData.get("scrapCost")
                    else "",
                ],
            )
        )
    ...
    def calculateUpgradePrice(params: dict, card: dict):
        if not params["rarityRatios"].get(str(card["rarity"])):
            return None
    ...
    def findUpgradeableCards(
        cards: list, params: dict, scrapCount: int = 0, repeats: bool = True
    ) -> list[dict]:
        cardData = [
            (card, cards.count(card), cidx)
            for cidx, card in enumerate(cards)
            if card not in cards[cidx + 1 :] and card["level"] < card["maxlevel"]
        ]
        ...
        for card, cardCount, cardn in cardData:
            if (
                not repeats
                or cardCount < params["repeats"][str(card["rarity"])][card["level"] - 1]
            ):
                cardCost = botUtils.calculateUpgradePrice(params["cost"], card)
                if cardCost is None or cardCost > scrapCount:
                    continue

                upgradeableCards.append({"index": cardn, "scrapCost": cardCost})
                continue
