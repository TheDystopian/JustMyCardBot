...
class cards:
    ...
    def getCardByRarity(self, cards: list[int] = None, chances: dict|int = None, level: int = 1):
        if chances is None:
            ...
            return {
                'id': choice(
                    ...
                    if chances != 0
                        ...
                        if rarity[0] != self.conf['game']['cards']['noRandomIndicator']
                    ]))
                ),
                'level': level
            }
        ...
        if cards is None:
            ...
            for cardID in cards:
                cds[str(self.conf['game']['cards']['cards'][cardID]['rarity'])].append(cardID)
            cards = cds
    ...
    def getOwnedCards(self, carddata: list, *, select: int|list = None, sort: str = None):
        ...
        returnable = [
            {
                ...
                "attachment": self.conf['game']['cards']['cards'][i["id"]]["photo"][f'lvl{i["level"]}'],
                'level': i['level'],
        ...
        return (
            returnable
            if not sort
            else sorted(returnable, key=itemgetter(sort))
        )